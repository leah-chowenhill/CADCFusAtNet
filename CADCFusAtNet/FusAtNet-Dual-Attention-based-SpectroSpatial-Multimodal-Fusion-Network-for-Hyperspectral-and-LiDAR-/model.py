...
train_patches = np.load('/content/drive/MyDrive/Perception_in_Snow/CADCFusAtNet/FusAtNet-Dual-Attention-based-SpectroSpatial-Multimodal-Fusion-Network-for-Hyperspectral-and-LiDAR-/data/train_patches/train_patches.npy')
test_patches = np.load('/content/drive/MyDrive/Perception_in_Snow/CADCFusAtNet/FusAtNet-Dual-Attention-based-SpectroSpatial-Multimodal-Fusion-Network-for-Hyperspectral-and-LiDAR-/data/test_patches/test_patches.npy')

train_labels = np.load('/content/drive/MyDrive/Perception_in_Snow/CADCFusAtNet/FusAtNet-Dual-Attention-based-SpectroSpatial-Multimodal-Fusion-Network-for-Hyperspectral-and-LiDAR-/data/train_labels/train_labels.npy')
test_labels = np.load('/content/drive/MyDrive/Perception_in_Snow/CADCFusAtNet/FusAtNet-Dual-Attention-based-SpectroSpatial-Multimodal-Fusion-Network-for-Hyperspectral-and-LiDAR-/data/test_labels/test_labels.npy')
...
train_hsi = train_patches[:,:,:,0:3]
train_lidar = np.expand_dims(train_patches[:,:,:,3], axis = 3) # Expanding dimension to preserve shape
                                                           #since only one band is present

test_hsi = test_patches[:,:,:,0:3]
test_lidar = np.expand_dims(test_patches[:,:,:,3], axis = 3)
...
with g.as_default():
  ...
  x1 = Input(shape=(11,11,3), name='inputA')     #num_hsi_bands = 3
  ...
  clsf = clf(at_feats, 2)                        # Classifier with number of classes = 2, road or not road
  ...
  for epoch in range(10):  # Number of epochs = 20
    ...
    if ovr_acc>=k:
      ...
      model_att.save('/content/drive/MyDrive/Perception_in_Snow/CADCFusAtNet/FusAtNet-Dual-Attention-based-SpectroSpatial-Multimodal-Fusion-Network-for-Hyperspectral-and-LiDAR-/models/model')
      k = ovr_acc
      ep = epoch
...
with g.as_default():
  ...
  # Loading saved model
  model = keras.models.load_model('/content/drive/MyDrive/Perception_in_Snow/CADCFusAtNet/FusAtNet-Dual-Attention-based-SpectroSpatial-Multimodal-Fusion-Network-for-Hyperspectral-and-LiDAR-/models/model')

  preds_final = model.predict([test_hsi, test_lidar], verbose = 1) # where train_hsi = train_rgb
